...
await app.register(swagger, {
    openapi: {
        ...
        info: {
            ...
            description: 'Servicio para consulta de reportes, metricas, alertas, auditorias y exportaciones.',
            version: '1.0.0',
            contact: {
                name: 'Equipo SCOPH'
            }
        ...
        tags: [
            ...
            { name: 'Reportes', description: 'Consultas operativas de inventario, jornadas y movimientos' },
            { name: 'Alertas', description: 'Alertas de stock bajo y vencimientos' },
            { name: 'Exportaciones', description: 'Descarga de reportes en Excel y PDF' },
            { name: 'Auditoria', description: 'Consulta de eventos auditados y consistencia de datos' }
        ],
        components: {
            securitySchemes: {
                bearerAuth: {
...
app.get(
    '/api/v1/health',
    {
        schema: {
            ...
            description: 'Verifica que el Report Service este corriendo correctamente.',
            response: {
                200: {
                    type: 'object',
                    properties: {
                        status: { type: 'string', example: 'ok' },
                        message: { type: 'string', example: 'Servicio de Reportes funcionando correctamente' },
