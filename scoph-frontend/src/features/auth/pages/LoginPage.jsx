...
// Elementos SVG flotantes médicos dentro del card izquierdo
    ...
    return (
        <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-l-3xl">
            ...
            <svg className="absolute bottom-16 left-0 right-0 opacity-[0.10]" width="100%" height="40" viewBox="0 0 400 40" preserveAspectRatio="none">
                <polyline points="0,20 40,20 52,20 60,4 68,36 76,10 84,30 92,20 150,20 165,20 173,6 181,34 189,12 197,28 205,20 270,20 285,20 293,5 301,35 309,13 317,30 325,20 400,20"
...
export default function LoginPage() {
    ...
    // Conectado al AuthService - POST /api/auth/login
    // Acepta username o correo + password
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            loginContext({ ...user, mustChangePassword }, token);

            // Si debe cambiar contraseña redirige a esa vista
    ...
    return (
        <div
            ...
            style={{ background: "linear-gradient(135deg, #F2F2F0 0%, #ffffff 100%)" }}>
                    ...
                    <div className="w-full max-w-sm">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="flex flex-col gap-1">
                                <label className="text-sm font-semibold text-gray-600">Usuario o correo</label>
                                <input
                                    type="text"
                                    name="username"
                                    value={form.username}
                                    onChange={handleChange}
                                    placeholder="dgomez o daniel@scoph.org"
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <button type="button" onClick={() => navigate("/recover-password")}
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full text-white font-bold py-3 rounded-xl transition duration-200 shadow-md hover:shadow-xl active:scale-95 text-base disabled:opacity-60 disabled:cursor-not-allowed"
                                style={{ background: "linear-gradient(135deg, #F27405 0%, #D97236 60%, #F29863 100%)" }}
                                onMouseEnter={e => { if (!loading) e.currentTarget.style.background = "linear-gradient(135deg, #D97236 0%, #8B3A0F 60%, #D97236 100%)"; }}
                                onMouseLeave={e => { if (!loading) e.currentTarget.style.background = "linear-gradient(135deg, #F27405 0%, #D97236 60%, #F29863 100%)"; }}
                            >
                                {loading ? (
