...
function FloatingElements() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-l-3xl">
      ...
      <svg
        className="absolute top-1/2 left-1/3 opacity-[0.08] animate-pulse"
      ...
      <svg
        className="absolute bottom-16 left-0 right-0 opacity-[0.10]"
        width="100%"
        height="40"
        viewBox="0 0 400 40"
        preserveAspectRatio="none"
      ...
      <svg
        className="absolute bottom-20 left-8 opacity-[0.10]"
        width="50"
        height="50"
        viewBox="0 0 80 80"
      >
      ...
      <svg
        className="absolute top-1/3 left-6 opacity-[0.10]"
        width="40"
        height="40"
        viewBox="0 0 24 24"
      >
...
function BackgroundElements() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      />
      <svg
        className="absolute top-16 right-24 opacity-[0.05]"
        width="50"
        height="50"
        viewBox="0 0 40 40"
      >
      ...
      <svg
        className="absolute bottom-20 left-20 opacity-[0.05]"
        width="35"
        height="35"
        viewBox="0 0 40 40"
      >
...
export default function LoginPage() {
  ...
  // ELIMINAMOS los useState locales de loading y error
  // y en su lugar, los extraemos directamente desde tu store de Zustand
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. Delegamos todo a Zustand. Él activa el loading y maneja los errores.
    const response = await login({
      username: form.username,
      password: form.password,
    });

    // 2. Zustand nos devuelve un objeto indicando si tuvo éxito
    if (response.success) {
      // 3. Si tuvo éxito, leemos el usuario recién guardado para saber a dónde mandarlo
  ...
  // ... (aquí continúa tu return con los formularios) ...
                ...
                onMouseEnter={(e) => {
                  if (!loading)
                    e.currentTarget.style.background =
                      "linear-gradient(135deg, #D97236 0%, #8B3A0F 60%, #D97236 100%)";
                }}
                onMouseLeave={(e) => {
                  if (!loading)
                    e.currentTarget.style.background =
