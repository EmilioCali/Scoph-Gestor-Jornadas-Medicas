...
import { getWorkdays, createWorkday, deleteWorkday } from "../../../shared/apis/workdayService";
import {
    getCentralInventory,
    getWorkdayInventory,
    registerTransfer,
...
function normalizeWorkdayInventoryItem(item) {
    const medicine = item.medicineId && typeof item.medicineId === "object" ? item.medicineId : {};
...
export function useWorkdayInventory() {
    ...
    const fetchBaseData = useCallback(async () => {
        ...
        setError(null);
        try {
            const [workdaysRes, centralRes] = await Promise.all([
                getWorkdays(),
                getCentralInventory(),
            ]);
            ...
            setError(err.response?.data?.message ?? "No se pudieron cargar las jornadas");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const fetchWorkdayInventory = useCallback(async (workdayId) => {
        ...
        try {
            ...
            setError(err.response?.data?.message ?? "No se pudo cargar el inventario de la jornada");
            return [];
        } finally {
    ...
    const createNewWorkday = useCallback(async (payload) => {
        const { data } = await createWorkday(payload);
        await fetchBaseData();
        return data.data;
    }, [fetchBaseData]);

    const removeWorkday = useCallback(async (workdayId) => {
        const { data } = await deleteWorkday(workdayId);
        await fetchBaseData();
        return data.data;
    }, [fetchBaseData]);

    const assignMedicine = useCallback(async ({ workday, medicineId, batch, quantity }) => {
        const selected = centralInventory.find((item) => String(item.medicineId) === String(medicineId));
    ...
    const registerWorkdayConsumption = useCallback(async ({ item, quantity }) => {
    ...
    const registerWorkdayReturn = useCallback(async ({ item, quantity }) => {
    ...
    const activeCentralInventory = useMemo(() =>
        centralInventory.filter((item) => item.totalStock > 0 && item.lots?.some((lot) => lot.stock > 0)),
    [centralInventory]);
